...
def load_scene_graph_from_json(scene_graph_json_path: str):
    with open(scene_graph_json_path, "r") as f:
        payload = json.load(f)
    if isinstance(payload, dict) and "scene_graph" in payload and isinstance(payload["scene_graph"], dict):
        return payload["scene_graph"]
    if isinstance(payload, dict) and "rooms" in payload:
        return payload
    raise ValueError(f"Invalid scene graph json format: {scene_graph_json_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--instruction",
                        action="store", type=str, dest="instruction", default="",
                        help="Optional instruction string for replan context")
    parser.add_argument("--scene-graph-json",
                        action="store", type=str, dest="scene_graph_json", default="",
                        help="Path to external scene graph JSON generated from vision")
    ...
    if (not args.scene_graph_json) and args.scene == args.scene_example:
    ...
    if (not args.scene_graph_json) and args.scene not in qry["scene"]:
    ...
    goal_qry = qry["goal"]
    gt_cost = qry["gt_cost"].get(args.scene, 0)
    ...
    for e in range(args.episode):
        ...
        if args.scene_graph_json:
            scene_qry = load_scene_graph_from_json(args.scene_graph_json)
        else:
