...
class L1RegressionActionHead(nn.Module):
    ...
    def predict_action(self, actions_hidden_states):
        ...
        input_device = actions_hidden_states.device
        input_dtype = actions_hidden_states.dtype
        module_device = next(self.parameters()).device
        module_dtype = next(self.parameters()).dtype
        actions_hidden_states = actions_hidden_states.to(device=module_device, dtype=module_dtype)
        rearranged_actions_hidden_states = actions_hidden_states.reshape(batch_size, NUM_ACTIONS_CHUNK, -1)
        action = self.model(rearranged_actions_hidden_states)
        return action.to(device=input_device, dtype=input_dtype)
...
class DiffusionActionHead(nn.Module):
    ...
    def predict_noise(self, actions_hidden_states):
        ...
        return noise_pred.to(device=input_device, dtype=input_dtype)
