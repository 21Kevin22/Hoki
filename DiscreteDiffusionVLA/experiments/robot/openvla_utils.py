...
def using_quantized_base_model(cfg: Any) -> bool:
    """Whether the backbone model is loaded with bitsandbytes quantization."""
    return bool(getattr(cfg, "load_in_8bit", False) or getattr(cfg, "load_in_4bit", False))


def get_auxiliary_device(cfg: Any) -> torch.device:
    """Choose where auxiliary modules such as the action head and proprio projector live."""
    if getattr(cfg, "offload_aux_modules_to_cpu", False):
...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    quantized = using_quantized_base_model(cfg)
    model_kwargs = dict(
    ...
    if quantized:
        # Quantized models are already placed by transformers/accelerate.
    ...
    try:
        ...
        if quantized:
            raise RuntimeError(
                "Failed to load quantized model. Check bitsandbytes / transformers / torch compatibility."
            ) from e
        raise
    ...
    # Quantized models are already dispatched by transformers/accelerate and must
    # not be moved afterwards with `.to(...)`.
    if not quantized:
        vla = vla.to(DEVICE)
...
def get_proprio_projector(cfg: Any, llm_dim: int, proprio_dim: int) -> ProprioProjector:
    ...
    """
    aux_device = get_auxiliary_device(cfg)
    aux_dtype = torch.float32 if aux_device.type == "cpu" else torch.bfloat16
    ...
    proprio_projector = proprio_projector.to(aux_dtype).to(aux_device)
...
def get_noisy_action_projector(cfg: Any, llm_dim: int) -> NoisyActionProjector:
    ...
    noisy_action_projector = noisy_action_projector.to(aux_dtype).to(aux_device)
...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead, DiffusionActionHead]:
    ...
    action_head = action_head.to(aux_dtype).to(aux_device)
    action_head.eval()
