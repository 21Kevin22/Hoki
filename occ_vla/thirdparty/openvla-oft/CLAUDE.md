...
- `../../scripts/run_libero_occluded_fast_scan.py` — fast (no per-step
  ...
  on the REAL `libero_10_occluded` suite (real deliberately-placed 3D
  occluder objects, registered via `../../scripts/register_libero_occ_suites.py`).
  Use this, not `run_libero_occ_benchmark.py`, for anything meant to compare
  against the published LIBERO-Occ paper's own numbers.
...
10. **Two real bugs found building a task-agnostic natural self-occlusion
    ...
    STATIC agentview camera, per [[feedback_segmentation_occlusion_measurement]])
    is invalid for the WRIST camera specifically, since it moves with the
    arm — the same screen-space region means a different 3D point every
    step. Fix: a LIVE per-step hide-and-reveal (two renders/step: current
    target-pixel count vs. robot-hidden target-pixel count, from the SAME
    current camera pose each time).
11. **A 10-task natural-self-occlusion baseline scan (n=20, plain
    ...
    **stalls on the SECOND pot during a LOWER-occlusion window** — a
    ...
    unlikely to be the right lever here.**
...
above — it wasn't, on real natural masks). A third probe predicting
...
codebase as a documented dead end, not deleted. Not yet done: none of
this dynamic-gating machinery has been tested against a REAL natural
occlusion mask (only synthetic `wrist_partial`) — same generalization gap
as finding 7's VJEPA-oracle result; worth checking before trusting it on
task_id=8 or `libero_10_occluded`.
