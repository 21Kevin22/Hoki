...
def check_model_logic_mismatch(pretrained_checkpoint: str) -> None:
    ...
    # Find current files
    # (includes vjepa_latent_predictor.py -- modeling_prismatic.py imports it
    # by absolute package path, but a *local-directory* checkpoint's own copy
    # of modeling_prismatic.py needs the sibling file physically present too,
    # in case that checkpoint dir is ever used standalone / with `trust_remote_code`
    # outside this editable-installed `prismatic` package.)
    curr_files = {
        "modeling_prismatic.py": None,
        "configuration_prismatic.py": None,
        "vjepa_latent_predictor.py": None,
    }
...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    # fully backward-compatible for every caller that doesn't set the attribute.
    ...
    # any checkpoint's state dict (they're newly added) and live NESTED
    # inside a submodule (vision_backbone) that DOES have checkpointed
    # weights -- confirmed via direct inspection that this combination
    # leaves them as literal META tensors (no real storage at all) after
    # `from_pretrained(..., low_cpu_mem_usage=True)`, not just
    # wrong-but-real garbage values (which is what the OLD, top-level-only
    # single-predictor design had -- a real, different failure mode for the
    # nested case). `nn.init.*` calls silently no-op on a meta tensor (no
    # error, but no real effect either), so `to_empty(device=...)` MUST run
    # first to actually allocate real storage, before `reset_parameters()`
...
def get_vla_action(
    ...
    Args:
        ...
        occlusion_mask: Optional (1, 256 * num_images, 1) tensor passed through
            to `vla.predict_action` for the VJEPA in-place occlusion overwrite.
            None (default) is fully backward-compatible.
        return_hidden_states: If True, also returns the mean-pooled
            action-token final-layer hidden state (np.ndarray, shape (D,))
            that `vla.predict_action` already computes internally (it's the
            LLM's own context representation right before the action head --
            not a new forward pass) -- for occ_vla's failure-prediction-probe
            work (2026-08-03), following arXiv:2606.29699's finding that
