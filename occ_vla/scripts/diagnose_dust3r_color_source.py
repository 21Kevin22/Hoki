...
def main():
    ...
    # diagnostic tint per source view: red for past_agentview, blue for
    # cur_wrist -- lets us see directly which view's points are winning
    # the black-blob region, instead of guessing
    view_tints = [np.array([255, 60, 60]), np.array([60, 60, 255])]
    all_pts_list, all_colors_list, all_confs_list, all_tints_list = [], [], [], []
    for i in range(2):
    ...
    for name, eye in grid.items():
        ...
        results.append({"name": name, "s_occ": s_occ, "coverage": coverage, "img": img, "cam2world": cam2world})
    ...
    # diagnostic: re-render the best candidate colored by SOURCE VIEW
    # (red=past_agentview, blue=cur_wrist) instead of real RGB, to see
    # directly whether the "black blob" region is dominated by
    # cur_wrist's own (dark gripper) points landing in the wrong place
    tint_img, _, _ = s_occ_score(all_pts, all_tints.astype(np.float64) / 255.0, all_confs, target_mask, best["cam2world"], focal_v, RENDER_SIZE, conf_thr)
    Image.fromarray(tint_img).save(OUT_DIR / f"mug_step{CUR_STEP:04d}_sourceview_diagnostic.png")
    print(f"saved source-view diagnostic (red=past_agentview, blue=cur_wrist): "
          f"{OUT_DIR / f'mug_step{CUR_STEP:04d}_sourceview_diagnostic.png'}", flush=True)
    ...
    sheet.save(OUT_DIR / f"mug_step{CUR_STEP:04d}_pastview_dedup_sheet.png")
    ...
    print(f"\nsaved: past_agentview | cur_agentview(occluded, display-only) | cur_wrist | {top4_labels} "
          f"-> {OUT_DIR / f'mug_step{CUR_STEP:04d}_pastview_dedup_sheet.png'}")
