"""User follow-up (2026-07-22): mug_in_microwave's wrist camera never
saw the target during its occlusion window (previous entry, CLAUDE.md)
-- try agentview's OWN past (pre-occlusion) frame as the second view
instead, since agentview is a static camera (confirmed: identical
cam_pos/cam_mat at step 70 vs step 110) -- same background, same pose,
just captured before the arm moved in. Also verify the grid candidates
the same way as the bowl_top_drawer success (test_dust3r_true_pose_grid_select.py).

3-image reconstruction: past-clear-agentview (step 70, arm_s_occ~0.01,
target clearly visible) + current-occluded-agentview (step 110) +
current-wrist (no target visible, included anyway for extra real
geometry/context, doesn't hurt). Same "Option 2" approach throughout:
dust3r's own cross-view pose estimation is not used at all (already
found broken when views share little content, and 3-image
PointCloudOptimizer alignment would face the same problem for the
occluded-agentview/wrist pair) -- only its PER-VIEW depth maps are used,
fused into a shared world frame via TRUE sim camera poses (agentview's
pose is identical at both timestamps by construction; wrist's is real
for the current step).

Run in .venv_mmada, one GPU:
  source .venv_mmada/bin/activate
  python3 scripts/test_dust3r_pastview_grid_select.py
"""
...
def main():
    ...
    filelist = [
        ...
        str(FRAME_DIR / f"step{CUR_STEP:04d}_agentview.png"),
        str(FRAME_DIR / f"step{CUR_STEP:04d}_wrist.png"),
    ]
    imgs = load_images(filelist, size=512, verbose=True)
    pairs = make_pairs(imgs, scene_graph="complete", prefilter=None, symmetrize=True)
    output = inference(pairs, model, device, batch_size=1, verbose=True)
    # 3 images -> PointCloudOptimizer mode; we still only use its PER-VIEW
    # depth output below, not its estimated relative poses.
    scene = global_aligner(output, device=device, mode=GlobalAlignerMode.PointCloudOptimizer, verbose=True)
    _ = scene.compute_global_alignment(init="mst", niter=200, schedule="cosine", lr=0.01)
    ...
    true_poses = [
        ...
        cam_to_world(cur_av_pose["cam_pos"], cur_av_pose["cam_mat"]),
        cam_to_world(cur_wrist_pose["cam_pos"], cur_wrist_pose["cam_mat"]),
    ]
    view_names = ["past_agentview", "cur_agentview", "cur_wrist"]
    ...
    real_cur_av = np.asarray(Image.fromarray((np.asarray(rgbimg[1]) * 255).astype(np.uint8)).resize((w, h)))
    real_wrist = np.asarray(Image.fromarray((np.asarray(rgbimg[2]) * 255).astype(np.uint8)).resize((w, h)))
    top4 = results[:4]
    ...
    sheet.save(OUT_DIR / f"mug_step{CUR_STEP:04d}_pastview_gridselect_sheet.png")
    top4_labels = ", ".join(f"{r['name']}(Socc={r['s_occ']:.2f})" for r in top4)
    print(f"\nsaved: past_agentview | cur_agentview(occluded) | cur_wrist | {top4_labels} "
          f"-> {OUT_DIR / f'mug_step{CUR_STEP:04d}_pastview_gridselect_sheet.png'}")
